        ...
        if self.rate_limit_worker:
            with ExitStack() as stack:
                ...
                try:
                    ...
                    logger.warning(f"Rate limiter acquire timeout or error: {e}, proceeding without rate limit")
                try:
...
class CustomSearchTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        try:
            result_text, metadata = await asyncio.wait_for(
                self.execution_pool.execute.remote(
                    self.execute_search, instance_id, query_list_from_params, self.topk, self.timeout
                ),
                timeout=120.0  # 120s hard deadline for Ray remote call (bocha retry needs up to 90s)
            )
